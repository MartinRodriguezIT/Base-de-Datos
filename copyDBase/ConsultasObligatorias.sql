...
DELIMITER $$
CREATE TRIGGER nombreControlarEquiposEnCompetencia
AFTER INSERT ON torneosTienenEncuentrosEquipos
FOR EACH ROW
BEGIN 
if (1 < (select count(*) from ParticipatesTorneosEquipos where ParticipatesTorneosEquipos.idEquipo=new.idEquipo AND ParticipatesTorneosEquipos.idTorneo=new.idTorneo))
then begin
	insert into ParticipatesTorneosEquipos(idTorneo, idEquipo, estadoEquipo) values (new.idTorneo, new.idEquipo, true);
end ; 
end if;
END$$ ;
DELIMITER ;
drop trigger nombreControlarEquiposEnCompetencia;
